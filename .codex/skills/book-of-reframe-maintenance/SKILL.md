...
2. **Inspect source.** Run the Copilot capability audit and inspect the registry, generated manifest, slash index, and
   command catalog codec. Search the router when a command's ownership is unclear.
3. **Drive.** For a changed command surface, run a governed fresh-store live drive. Use AX for interaction and
...
4. **Extract.** Read the persisted `app.commands.discover` aggregate with `--corpus`, then run
...
5. **Reconcile.** Compare command entries with `docs/copilot-capability-audit.md` and the registry. Record commands
   that are aliases, local orchestration, unavailable, `//`-only, or lacking a governed capability identity.
6. **Teach.** Add human-readable explanations, examples, required state, cost/confirmation, persisted effects, AX
   ...
   auditable.
7. **Validate.** Run the export check, capability audit, governance validator, focused tests, `git diff --check`, and
   `Scripts/sync-agent-skills --check`. If runtime or UI changed, run the relevant build/live proof as well.
8. **Publish.** Push the reviewed projection to `Fountain-Coach/book-of-reframe`. Link it back to
...
9. **Record.** Update the integration `PLANS.md` and the book's `SOURCE.md` with snapshot date, source commit, live
